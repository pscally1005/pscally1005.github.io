---
layout: post
title:  Sourdough Discard Flatbread
date:   2026-03-01 00:00:00 -0500
categories:
- Recipes
- Bread
showPercentage: no
hummus: no
oatmeal: no
yogurt: no
nut_butter: no
pesto: no
soup_stew: no
salad: no
dressing: no
brownie: no
cookie: no
copycat: no
vic: no
protein_bar: no
permalink: /recipes/sourdough-flatbread
image: /assets/Food/Bread/Discard flatbread/cover.jpg
file: sourdoughflatbread
servingSize: 1 flatbread (48 g)
section1:
start2:
section2:
start3:
section3:
start4:
section4:
start5:
section5:
Prep: 5
Rest:
Cook: 10
Source1: https://twelveonmain.com/2-ingredient-easy-sourdough-flatbread/
Source2: https://www.poormanprotein.com/misc/whole-wheat-sourdough
Source3:
Source4:
Source5:
whisk: https://s.samsungfood.com/AHZEc
tags:
- sourdough
- whole wheat
- flatbread
- tortilla
- starter
Description: |
    Every 2 weeks, I feed my <a href='/misc/whole-wheat-sourdough#starter'>whole wheat sourdough starter</a> with 1/2 cup (60 g) <a href='/misc/grains#whole-wheat'>whole wheat flour</a> and 1/4 cup (60 g) of water, discarding half the existing mixture.<br><br>

    When I'm not in the mood to make another loaf of <a href='/misc/whole-wheat-sourdough#bread-recipe'>sourdough</a>, I utilize my 1/2 cup (120 g) of <a href='/misc/whole-wheat-sourdough#starter'>sourdough starter</a> to make these flatbreads, that way there's no waste!  Just combine the <a href='/misc/whole-wheat-sourdough#starter'>starter</a> with salt and baking powder, and cook on the stove like a pancake.
Instructions:
- Preheat a cast iron or nonstick pan over medium heat with a <a href='https://amzn.to/3Hdg0gk'>light spray of oil</a>

- In a <a href='https://amzn.to/4adxMft'>small bowl</a>, combine together your <a href='/misc/whole-wheat-sourdough#starter'>starter</a> with salt and baking powder with a <a href='https://amzn.to/40F2aK0'>silicone spatula</a>

- When the pan is hot, briefly remove it from the heat.  Add 1/2 cup (half the mixture, 120 g) into the pan, and use a <a href='https://amzn.to/40F2aK0'>silicone spatula</a> to spread it into a 6 inch round flatbread. Add any toppings you like, such as <a href='/misc/dairy#mozzarella'>cheese</a> or everything bagel seasoning
- <img src='/assets/Food/Bread/Discard flatbread/cooking1.jpg' alt='' class='instruction-image'>

- Return the pan to the burner over medium heat.  Cook for about 2 minutes, or until the top side is dry, has small holes, and has lightly risen, and the bottom is lightly browned.  Use a spatula to remove it to a plate.  Repeat with the other flatbread
- <img src='/assets/Food/Bread/Discard flatbread/cooking2.jpg' alt='' class='instruction-image'>
Notes:
---
